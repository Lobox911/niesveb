  ...
  return (
    <footer className="mt-24 bg-ink text-white">
      <div className="container-content grid gap-10 py-14 md:grid-cols-3">
        <div>
          <h3 className="text-[20px]">{branch.branchName}</h3>
          <p className="mt-3 text-[15px] text-white/70">{event?.title ?? ""}</p>
          ...
          {branch.contactPhones.length > 0 && (
            <ul className="mt-4 space-y-1">
              {branch.contactPhones.map((p: string) => (
                <li key={p}>
          ...
          {branch.contactEmail && (
            <a className="mt-2 block text-[14px] text-white/80 hover:underline" href={`mailto:${branch.contactEmail}`}>
              {branch.contactEmail}
            </a>
          )}
        </div>
      ...
      <div className="border-t border-white/15">
        <div className="container-content py-6">
          <p className="text-[14px] text-white/70">
            © {currentYear} {branch.branchName}. All rights reserved.
          </p>
          {branch.registeredAddress && (
            <p className="mt-1 text-[13px] text-white/50">{branch.registeredAddress}</p>
          )}
        </div>
      </div>
    </footer>
