...
type Cat = { id: string; name: string; eligibility: string; fee: number; units: number; requiresMembershipNo: boolean; sortOrder: number };
...
export function CategoryList({ rows }: { rows: Cat[] }) {
  return (
    <EditableList
      ...
      rows={rows.map((c) => ({
        ...c,
        summary: c.name,
        detail: `${c.id} · ${naira(c.fee)} · ${c.units} units`,
      }))}
      fields={[
        { name: "id", label: "Id", mono: true, width: "half", help: "Lowercase, hyphens. Cannot change later." },
...
export function AdvertList({ rows }: { rows: Adv[] }) {
  return (
    <EditableList
      ...
      onSave={saveAdvertRate}
      onDelete={deleteAdvertRate}
    />
  );
}

export function ProgrammeList({ rows }: { rows: Prog[] }) {
  return (
    <EditableList
      title="Programme"
